function updateSaoPauloTime() {
    ...
    const SaoPauloTimeZoneOffset = -0 * 60;
    const now = new Date();
    now.setMinutes(now.getMinutes() + SaoPauloTimeZoneOffset);
    const timeString = now.toLocaleTimeString("en-US", { timeZone: 'America/Sao_Paulo' });
    SaoPauloTimeElement.textContent = timeString;
}
...
function updateCountdownTimer() {
    ...
    const currentDateTime = new Date(
        now.toLocaleString("en-US", { timeZone: SaoPauloTimeZone })
    );
