function updateSydneyTime() {
    const SydneyTimeElement = document.getElementById("SydneyTime");
    const SydneyTimeZoneOffset = -0 * 60; 
    const now = new Date();
    now.setMinutes(now.getMinutes() + SydneyTimeZoneOffset);
    const timeString = now.toLocaleTimeString("en-US", { timeZone: 'Australia/Sydney' });
    SydneyTimeElement.textContent = timeString;
}
...
function updateCountdownTimer() {
    ...
    const currentDateTime = new Date(
        now.toLocaleString("en-US", { timeZone: SydneyTimeZone })
    );
